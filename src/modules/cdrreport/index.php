...
function renderDirectionTrunkBadge($did, $raw_src, $val_dst, $channel = '', $dstchannel = '', $entities = array()) {
    ...
    $extMap    = isset($entities['extensions']) ? $entities['extensions'] : (is_array($entities) ? $entities : array());
    $queuesMap = isset($entities['queues']) ? $entities['queues'] : array();
    $rgMap     = isset($entities['ringgroups']) ? $entities['ringgroups'] : array();

    $srcIsExt = (strlen($cleanSrc) >= 2 && strlen($cleanSrc) <= 5 && is_numeric($cleanSrc)) || isset($extMap[$raw_src]) || isset($extMap[$cleanSrc]);
    $dstIsExt = (strlen($cleanDst) >= 2 && strlen($cleanDst) <= 5 && is_numeric($cleanDst)) || isset($extMap[$val_dst]) || isset($extMap[$cleanDst]);
    $dstIsQueue = isset($queuesMap[$val_dst]);
    $dstIsRingGroup = isset($rgMap[$val_dst]);
    ...
    // Caso 1: Destino 's' (Atendimento Automático / URA)
    if (strtolower($val_dst) === 's') {
        return "<span title='🎛️ Atendimento Automático / URA do PBX' style='background:#fef3c7; color:#b45309; border:1px solid #fde68a; padding:4px 10px; border-radius:12px; font-weight:700; font-size:11px; display:inline-flex; align-items:center; gap:5px;'><i class='fa fa-sitemap'></i> Sistema / URA</span>";
    }

    // Caso 2: Captura de Ramal (00, *8, **)
    if ($val_dst === '00' || $val_dst === '*8' || $val_dst === '**') {
        return "<span title='📥 Captura de Chamada (Puxar Ligação)' style='background:#e0f2fe; color:#0369a1; border:1px solid #bae6fd; padding:4px 10px; border-radius:12px; font-weight:700; font-size:11px; display:inline-flex; align-items:center; gap:5px;'><i class='fa fa-phone-square'></i> Captura</span>";
    }

    // Caso 3: Fila de Atendimento
    if ($dstIsQueue) {
        $qName = $queuesMap[$val_dst];
        $sub = !empty($qName) ? " <span style='font-size:10px; color:#5b21b6; font-weight:600;'>(" . htmlspecialchars($qName) . ")</span>" : "";
        return "<span title='👥 Entrada via Fila de Atendimento: " . htmlspecialchars($val_dst . (!empty($qName) ? " - $qName" : ""), ENT_QUOTES) . "' style='background:#ede9fe; color:#6d28d9; border:1px solid #ddd6fe; padding:4px 10px; border-radius:12px; font-weight:700; font-size:11px; display:inline-flex; align-items:center; gap:5px;'><i class='fa fa-users'></i> Fila$sub</span>";
    }

    // Caso 4: Grupo de Chamadas (Ring Group)
    if ($dstIsRingGroup) {
        $gName = $rgMap[$val_dst];
        $sub = !empty($gName) ? " <span style='font-size:10px; color:#075985; font-weight:600;'>(" . htmlspecialchars($gName) . ")</span>" : "";
        return "<span title='🏢 Entrada via Grupo de Ramais: " . htmlspecialchars($val_dst . (!empty($gName) ? " - $gName" : ""), ENT_QUOTES) . "' style='background:#e0f2fe; color:#0369a1; border:1px solid #bae6fd; padding:4px 10px; border-radius:12px; font-weight:700; font-size:11px; display:inline-flex; align-items:center; gap:5px;'><i class='fa fa-building'></i> Grupo$sub</span>";
    }

    // Caso 5: Entrada com DID direto
    if (!empty($did) && $did != '-') {
    ...
    // Caso 6: Saída (Ramal para número externo)
    if ($srcIsExt && !$dstIsExt && !empty($val_dst) && $val_dst != '-') {
    ...
    // Caso 7: Chamada Interna (Ramal para Ramal)
...
function renderFullCdrDashboard($oCDR, $pDB, $module_name, $smarty)
{
    ...
    $pDB_asterisk = new paloDB($dsn_asterisk);
    $entitiesMaps = getAsteriskEntitiesMaps($pDB_asterisk);
    $dataRG       = $entitiesMaps['ringgroups'];
    $dataQueue    = $entitiesMaps['queues'];

    if (class_exists('RingGroup')) {
        $oRG = new RingGroup($pDB_asterisk);
        $resRG = $oRG->getRingGroup();
        if (is_array($resRG)) $dataRG = $dataRG + $resRG;
    }
    if (class_exists('Queue')) {
        $oQueue = new Queue($pDB_asterisk);
        $resQ = $oQueue->getQueue();
        if (is_array($resQ)) $dataQueue = $dataQueue + $resQ;
    }
    $groupsMap = $dataRG + $dataQueue;
    $entitiesMaps['all_groups'] = $groupsMap;
