    ...
    if (number === 1) {
        notificationContainer.innerHTML = `
            <div style="position: relative;">
                <p class="notification-message">Voce deve selecionar uma extensao de destino!</p>
            </div>
        `;
    } else if (number === 2) {
        notificationContainer.innerHTML = `
            <div style="position: relative;">
                <p class="notification-message">Voce n�o pode selecionar sua propria extens�o como destino!</p>
            </div>
        `;
    } else if (number === 3) {
        notificationContainer.innerHTML = `
            <div style="position: relative;">
                <p class="notification-message">O ramal selecionado n�o tem chamadas nesta linha!</p>
            </div>
        `;
    } else if (number === 4) {
        notificationContainer.innerHTML = `
            <div style="position: relative;">
                <p class="notification-message">Sua extens�o n�o est� conectada para executar esta a��o!</p>
            </div>
        `;
    }else if (number === 5) {
        notificationContainer.innerHTML = `
            <div style="position: relative;">
                <p class="notification-message">Sua conta n�o tem nenhuma extens�o associada a ela!</p>
            </div>
        `;
    }
    ...
    data.forEach(queueData => {
        ...
        if (divConDataIDAttr) {
            ...
            if (queueParameters) {
            	var html = '';
            	html += '<div>';
							html += '<span class="answered" style="margin-right: 20px"><strong title="Esses dados sao atualizados a cada 5 minutos.">Atendido:</strong> (' + queueData.Completed + ')</span>';
							html += '<span class="abandoned"><strong title="Esses dados sao atualizados a cada 5 minutos.">Nao-Atendido:</strong> (' + queueData.Abandoned + ')</span>';
							html += '</div>';
...
function memberStatus(status, paused) {
    if (paused !== "0" && status !== "5") {
        return {
            ...
            circleHTML: '<div class="circle green" title="Em Pausa (DND - Nao Pertube Ativado)"></i></div>'
        };
    }
    ...
    switch (status) {
        case '1':
            return {
                statusImage: '<img src="/modules/'+module_name+'/images/agent-available.png" alt="Disponivel" style="padding-right:1px;"/>',
                circleHTML: '<span class="circle green" title="Disponivel"></span>'
            };
        case '2':
            return {
        ...
        case '6':
            return {
                statusImage: '<img src="/modules/'+module_name+'/images/agent-ringing.gif" alt="Chamando" style="padding-right:1px;"/>',
                circleHTML: '<div class="circle yellow" title="Chamando"></div>'
            };
        case '7':
            return {
                statusImage: '<img src="/modules/'+module_name+'/images/agent-busy.png" alt="Chamando / Em Uso" style="padding-right:1px;"/>',
                circleHTML: '<div class="circle orange-yellow" title="Chamando / Em Uso"></div>'
            };
        case '8':
            return {
                statusImage: "<i class='fa fa-stop' style='padding-right:4px; padding-left:1px; font-size:13px'></i>",
                circleHTML: '<div class="circle orange" title="Em Espera"></div>'
            };
        default:
            return {
                statusImage: "",
