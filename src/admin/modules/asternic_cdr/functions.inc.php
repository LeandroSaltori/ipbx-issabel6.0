...
if(!isset($seriename['A'])) {
    if(preg_match("/secs/",$options['title'])) {
        $seriename['A']="Segundos";
    } else {
        $seriename['A']="Contagem";
    }
}
...
function print_exports($header_pdf,$data_pdf,$width_pdf,$title_pdf,$cover_pdf,$appconfig) {
    ...
    echo _('Exportar para CSV/Excel');
    echo "</span></a>\n";
    echo "</form>";
}
...
function asternic_getrecords( $MYVARS ,$appconfig) {
    ...
    while (is_array($row = $res->fetchRow(DB_FETCHMODE_ASSOC))) {
        if (!(substr($row['accountcode'],0,5)=='Local' && $dispo[$row['disposition']]=='BUSY' && $row[9]=='ResetCDR')) {
            ...
            if($gtype=='combined') {
                if($row['chan1']<>$channel) { $direction='Entrada'; } else { $direction='Saida'; }
            ...
            $detail[$campo].="<td align=right>".$row['ringtime']." "._('seg')."</td>\n";
            ...
            if($row['recordingfile']<>"") {
                ...
                $detail[$campo].="<div class='playicon' title='"._('Ouvir')."' id='play".$uni."'  style='float:left;'>";
                ...
                $detail[$campo].="<div class='downicon' title='"._('Baixar')."' id='dload".$uni."'  style='float:left;'>";
                $detail[$campo].="<img src='images/blank.gif' alt='pixel' height='16' width='16' border='0'>";
                $detail[$campo].="</div></a>";
            } else {
                $detail[$campo].= "&nbsp;";
    ...
    if($gtype=='combined') {
        echo "<td bgcolor='#ddcc00'>Direcao</td>";
    }
    echo "<td bgcolor='#ddcc00' align='center'>"._('Data')."</td>\n";
    echo "<td bgcolor='#ddcc00'>"._('De')."</td>\n";
    echo "<td bgcolor='#ddcc00'>"._('Para')."</td>\n";
    echo "<td bgcolor='#ddcc00' align='right'>"._('Tempo da Chamada')."</td>\n";
    echo "<td bgcolor='#ddcc00' align='right'>"._('Tempo de Toque')."</td>\n";
    echo "<td bgcolor='#ddcc00' align='center'>"._('Status')."</td>\n";
    echo "<td bgcolor='#ddcc00' align='center'>"._('Ouvir')."</td></tr></thead>\n";
...
class PDF extends FPDF
{
    function Footer() {
        ...
        $this->Cell(0,10,$lang["$language"]['pagina'].' '.$this->PageNo(),0,0,'C');
    }
