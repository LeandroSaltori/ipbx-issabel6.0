...
install_openvpn() {
    ...
    echo ""
    log_info "Roteamento: Split-Tunneling ATIVO (A internet local do seu PC continua 100% normal e NUNCA cai)."
    ROUTING_CHOICE="1"
...
EOF
    ...
    # No modo Split-Tunneling (Padrão e Recomendado):
    # NUNCA redireciona o gateway e NUNCA altera o DNS do computador do usuário.
    # A internet do PC continua 100% normal pela própria rede dele.
    # Apenas os pacotes destinados ao PBX (VPN_SUBNET) entram no túnel.
    echo "push \"route $VPN_SUBNET $VPN_NETMASK\"" >>/etc/openvpn/server.conf
    # Adiciona também a rota da rede local do PBX caso seja diferente da VPN
    LOCAL_PBX_NET=$(ip -4 route show dev $NIC 2>/dev/null | grep -Po '^[0-9]+\.[0-9]+\.[0-9]+\.0/[0-9]+' | head -1 || true)
    if [[ -n "$LOCAL_PBX_NET" ]]; then
        LOCAL_IP_BASE=$(echo "$LOCAL_PBX_NET" | cut -d'/' -f1)
        echo "push \"route $LOCAL_IP_BASE 255.255.255.0\"" >>/etc/openvpn/server.conf
    fi
